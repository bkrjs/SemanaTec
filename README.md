<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<html><head><title>Python: module snake</title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
</head><body bgcolor="#f0f0f8">

<table width="100%" cellspacing=0 cellpadding=2 border=0 summary="heading">
<tr bgcolor="#7799ee">
<td valign=bottom>&nbsp;<br>
<font color="#ffffff" face="helvetica, arial">&nbsp;<br><big><big><strong>snake</strong></big></big></font></td
><td align=right valign=bottom
><font color="#ffffff" face="helvetica, arial"><a href=".">index</a><br><a href="file:/Users/rebecarojas/Desktop/SemanaTec/SemanaTec/snake.py">/Users/rebecarojas/Desktop/SemanaTec/SemanaTec/snake.py</a></font></td></tr></table>
    <p><tt>Authors:&nbsp;Authors:&nbsp;Rebeca&nbsp;Rojas&nbsp;Pérez&nbsp;A01751192<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Juan&nbsp;Carlos&nbsp;Jiménez&nbsp;Tapia&nbsp;A01750115<br>
&nbsp;<br>
Snake,&nbsp;classic&nbsp;arcade&nbsp;game.</tt></p>
<p>
<table width="100%" cellspacing=0 cellpadding=2 border=0 summary="section">
<tr bgcolor="#eeaa77">
<td colspan=3 valign=bottom>&nbsp;<br>
<font color="#ffffff" face="helvetica, arial"><big><strong>Functions</strong></big></font></td></tr>
    
<tr><td bgcolor="#eeaa77"><tt>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</tt></td><td>&nbsp;</td>
<td width="100%"><dl><dt><a name="-change"><strong>change</strong></a>(x, y)</dt><dd><tt>Change&nbsp;snake&nbsp;direction.</tt></dd></dl>
 <dl><dt><a name="-foo"><strong>foo</strong></a>(...)</dt><dd><tt><a href="#-foo">foo</a>(i,j)<br>
&nbsp;<br>
Return&nbsp;the&nbsp;sum&nbsp;of&nbsp;i&nbsp;and&nbsp;j.</tt></dd></dl>
 <dl><dt><a name="-inside"><strong>inside</strong></a>(head)</dt><dd><tt>Return&nbsp;True&nbsp;if&nbsp;head&nbsp;inside&nbsp;boundaries.</tt></dd></dl>
 <dl><dt><a name="-move"><strong>move</strong></a>()</dt><dd><tt>Move&nbsp;snake&nbsp;forward&nbsp;one&nbsp;segment&nbsp;at&nbsp;a&nbsp;time.</tt></dd></dl>
 <dl><dt><a name="-moveFood"><strong>moveFood</strong></a>()</dt><dd><tt>"Move&nbsp;food&nbsp;randomly&nbsp;one&nbsp;step&nbsp;at&nbsp;a&nbsp;time.</tt></dd></dl>
</td></tr></table><p>
<table width="100%" cellspacing=0 cellpadding=2 border=0 summary="section">
<tr bgcolor="#55aa55">
<td colspan=3 valign=bottom>&nbsp;<br>
<font color="#ffffff" face="helvetica, arial"><big><strong>Data</strong></big></font></td></tr>
    
<tr><td bgcolor="#55aa55"><tt>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</tt></td><td>&nbsp;</td>
<td width="100%"><strong>aim</strong> = vector(0, -10)<br>
<strong>colors</strong> = ['black', 'pink', 'green', 'blue', 'purple']<br>
<strong>food</strong> = vector(-30, 20)<br>
<strong>foodColor</strong> = 'orange'<br>
<strong>snake</strong> = [vector(10, -190)]<br>
<strong>snakeColor</strong> = 'green'</td></tr></table>
</body></html>